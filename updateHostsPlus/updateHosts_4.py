...
def process_hosts():
    global auto 
    if auto==1 or os.path.exists('hosts_from_web'):
        if auto==1 or any(open("hosts_from_web").read().strip()):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-a','--auto',
                        ...
                        nargs='?',
                        type=int,
                        help="auto check whether hosts exists.")    
    args = parser.parse_args()
    if args.test!=0:test=args.test
    if args.auto==0:
        if os.path.exists('hosts.txt'):auto=1
    main()
